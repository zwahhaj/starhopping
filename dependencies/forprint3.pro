pro forprint3, v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12, v13, v14, $
      v15,v16,v17,v18,TEXTOUT = textout, FORMAT = format, SILENT = SILENT, $ 
      STARTLINE = startline, NUMLINE = numline, outfile = outfile, $
      update = update, comments = comments, nospacecomments=nospacecomments
...
;
;       prints 1st line as a comment ("#")   
;       added 'outfile' as a synonym for 'textout'
;       renamed FORPRINT2.PRO         M. Liu  08/31/01
;       
;       write comments to file via the 'comments' string array  M. Liu 09/11/01 
;       added /update, now uses TEXTOPEN2.PRO   M. Liu 09/21/01 
;       added /nospacecomments                  M. Liu 09/27/05
;       changed text output width to 255 cols   Z. Wahhaj 07/21/11
;-            
  ;;openw,1,'output.txt',width=255
...
; Use default output dev.
  if keyword_set(outfile) then textout = outfile
  ...
  textopen3,'FORPRINT3',TEXTOUT=textout, update = update, silent = silent
  if keyword_set(comments) then begin ; print comments
                                ;if ( text_out gt 2) and keyword_set(comments) then begin  ; print comments
     if not(isstring(comments)) then $
        message, 'ERROR - comment field not passed as string(s)!'
     for i = 0, n_elements(comments)-1 do  $
        printf, !textunit, '# ', comments(i)
     if not(keyword_set(nospacecomments)) then $
        printf, !textunit 
  endif else if ( text_out GT 2 ) and (not keyword_set(SILENT)) and  $
     not(keyword_set(update)) then $ 
        printf,!TEXTUNIT,'# FORPRINT: ',systime(), userid()

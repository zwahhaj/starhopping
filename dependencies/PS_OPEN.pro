...
;-      070826 Zahed - added some defaults, so can call ps_open,'xxx.ps'
;-      080409 Zahed - added mpage keyword, to set bigger top/right margins
;  for handling the 'convert' cropping problem when selecting frames.

if (n_elements(color) eq 0) then color = 1
if (n_elements(thick) eq 0) then thick = 3
if (n_elements(ps_fonts) eq 0) then ps_fonts = 1
if (n_elements(portrait) eq 0) then portrait = 1
if n_elements(square) then portrait = 0
...
;	so large.
;
    if n_elements(square) then begin
       xoff = .7
       yoff = 3.6
       xsize = 6.8
       ysize = 6.8
       Device, /Portrait
       print, 'PORTRAIT'
    endif else If (portrait eq 1) Then Begin
       if (keyword_set(mpage)) then begin
          xoff = 0 & yoff = 0 & xsize = 5.0 & ysize = 7.0
          Device, /Portrait & print, 'PORTRAIT'
       endif else begin
          xoff = 1.0
          yoff = 1.0
          xsize = 6.5
    ...
    EndIf Else Begin		;/Landscape is the default
       if (keyword_set(mpage)) then begin
          xoff = 0 & yoff = 0 & xsize = 7.0 & ysize = 5.0
          Device, /Landscape & print, 'LANDSCAPE'
       endif else begin
          xoff = 1.0
          yoff = 10.0
          xsize = 9.0

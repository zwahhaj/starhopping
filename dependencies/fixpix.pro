...
; 04/25/01 M.Liu (IfA/Hawaii)
;
; checks to see if image is unusally small
; if so, then reduce 'npix' to a reasonable value
; 12/12/01 MCL
;-
...
;	print,format='(" ",A)',strc(j)
  ...
  ; check for very small image
  wg = where(bp ne 0.0, ng)
  if (ng) lt npix then begin
      message, 'unusally small image', /info
      npix = ng-1
  endif

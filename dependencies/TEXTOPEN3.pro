PRO TEXTOPEN3,PROGRAM,TEXTOUT=TEXTOUT, STDOUT = STDOUT, update = update, silent = silent
;+
; NAME:
;       TEXTOPEN
; PURPOSE:
...
;       Added /append, renamed TEXTOPEN2.PRO   M. Liu Sept 2001
;-
;-----------------------------------------------------------
  ;
  ; Open proper unit.
  ...
  case text_out of
     ...
     6: begin
         if not(keyword_set(update)) then begin
             openw,!TEXTUNIT,filename, width=255 
             if not(keyword_set(silent)) then  $
               message,'Output is being directed to a file "' + filename +'"',/INFORM
         endif else begin
             openw,!TEXTUNIT,filename, /append, width=255
             if not(keyword_set(silent)) then  $
               message,'Output is being appended to an existing file "' + filename + '"',/INFORM
         endelse
        end

     7: begin
        oname = strlowcase(strtrim( PROGRAM,2) +'.prt')
        openw, !TEXTUNIT, oname, /append, width=255
        if not(keyword_set(silent)) then  $
          message,'Output is being appended to file "' + oname + '"',/INFORM
        for i=0,3 do printf,!textunit,' '       ;added a couple of blank lines
        end

     0: openw,!TEXTUNIT, strtrim(PROGRAM,2) + '.tmp',/DELETE, width=255

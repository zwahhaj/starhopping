...
pro irdis_normfluxfac, filelist, normfac, dit, ndfac, lambdas, dlambdas,outsave=outsave 
  pname = 'irdis_normfluxfac'
  if n_elements(outsave) eq 0 then outsave='normscifac.save'
  
  readcol,filelist,filename,f='A'
...
case 1 of
   ...
   strmatch(ndfilter,'*1.0*') and strmatch(filter2,'*FILT_CLEAR*') and strmatch(filter1,'*FILT_BBF_H*'):  ndfac= 7.18
   strmatch(ndfilter,'*2.0*') and strmatch(filter2,'*FILT_CLEAR*') and strmatch(filter1,'*FILT_BBF_H*'):  ndfac= 56.1
   strmatch(ndfilter,'*3.5*') and strmatch(filter2,'*FILT_CLEAR*') and strmatch(filter1,'*FILT_BBF_H*'):  ndfac= 955.0

   strmatch(ndfilter,'*1.0*') and strmatch(filter2,'*FILT_CLEAR*') and strmatch(filter1,'*FILT_BBF_J*'):  ndfac= 15.9
   strmatch(ndfilter,'*2.0*') and strmatch(filter2,'*FILT_CLEAR*') and strmatch(filter1,'*FILT_BBF_J*'):  ndfac= 296.2
   strmatch(ndfilter,'*3.5*') and strmatch(filter2,'*FILT_CLEAR*') and strmatch(filter1,'*FILT_BBF_J*'):  ndfac= 16323.0

   strmatch(ndfilter,'*1.0*') and strmatch(filter2,'*FILT_CLEAR*') and strmatch(filter1,'*FILT_BBF_Y*'):  ndfac= 10.7
   strmatch(ndfilter,'*2.0*') and strmatch(filter2,'*FILT_CLEAR*') and strmatch(filter1,'*FILT_BBF_Y*'):  ndfac= 122.6
   strmatch(ndfilter,'*3.5*') and strmatch(filter2,'*FILT_CLEAR*') and strmatch(filter1,'*FILT_BBF_Y*'):  ndfac= 3349.7
   else: ndfac=1.0
endcase
...
case 1 of
strmatch(filter2,'*FILT_CLEAR*') and strmatch(filter1,'*FILT_BBF_H*'):  begin & lambda1 = 1.625 & lambda2 = 1.625 & dl1 = 0.290 & dl2 = 0.290 & end
...
strmatch(filter2,'*FILT_CLEAR*') and strmatch(filter1,'*FILT_BBF_Y*'):  begin & lambda1 = 1.043 & lambda2 = 1.043 & dl1 = 0.140 & dl2 = 0.140 & end
strmatch(filter2,'*FILT_DBF_Y23*') and strmatch(filter1,'*FILT_BBF_Y*'):  begin & lambda1 = 1.022 & lambda2 = 1.076 & dl1 = 0.049 & dl2 = 0.050 & end

strmatch(filter2,'*FILT_CLEAR*') and strmatch(filter1,'*FILT_BBF_J*'):  begin & lambda1 = 1.245 & lambda2 = 1.245 & dl1 = 0.240 & dl2 = 0.240 & end
strmatch(filter2,'*FILT_DBF_J23*') and strmatch(filter1,'*FILT_BBF_J*'):  begin & lambda1 = 1.190 & lambda2 = 1.273 & dl1 = 0.042 & dl2 = 0.046 & end

strmatch(filter2,'*FILT_CLEAR*') and strmatch(filter1,'*FILT_BBF_Ks*'):  begin & lambda1 = 2.182 & lambda2 = 2.182 & dl1 = 0.300 & dl2 = 0.300 & end
...
comment='"normflux" is the predicted flux for no ND, 1um band width, and 1s expo, given the original flux was 1 ADU. "peak" gives the unconverted peak flux of the PSFs.'
